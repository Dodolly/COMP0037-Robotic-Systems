    ...
    def evaluate_policy(self):
        ...
        for iteration in range(self._max_policy_evaluation_steps):
            ...
            for s in range(RobotBatteryState.NUMBER_OF_STATES):
                ...
                for a in range(RobotActions.NUMBER_OF_ACTIONS):
                    # Discard zero probability actions
                    if pia[a] == 0:
                        continue

                    # Compute p(s', r|s, a)
                    s_prime, r, p = environment.next_state_and_reward_distribution(s, a)

                    for t in range(len(p)):
                        if p[t] > 0: 
                            next_contribution = pia[a] * p[t] * (r + self._gamma * self._v[s_prime[t]])
                            new_v += next_contribution
